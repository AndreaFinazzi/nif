...
public:
  ...
  void pushBackDerivatives_d(double zeroth_derivative,
                             double first_derivative,
                             double second_drivative,
                             double third_derivative);
  void pushBackDerivatives_s(double zeroth_derivative,
                             double first_derivative,
                             double second_drivative,
                             double third_derivative);
  ...
  void push_back_point_x(double point_x) {
    points_x_.push_back(point_x);
  }
  void push_back_point_y(double point_y) {
    points_y_.push_back(point_y);
  }
  void push_back_yaw(double yaw) {
    yaw_.push_back(yaw);
  }
  ...
  void slice_point_x(int slice_vec_idx_) {
    points_x_ = std::vector<double>(points_x_.begin(),
                                    points_x_.begin() + slice_vec_idx_ + 1);
  }
  void slice_point_y(int slice_vec_idx_) {
    points_y_ = std::vector<double>(points_y_.begin(),
                                    points_y_.begin() + slice_vec_idx_ + 1);
  }
  void slice_yaw(int slice_vec_idx_) {
    yaw_ = std::vector<double>(yaw_.begin(), yaw_.begin() + slice_vec_idx_ + 1);
  }
  void slice_delta_velocity(int slice_vec_idx_) {
    delta_velocity_ = std::vector<double>(
        delta_velocity_.begin(), delta_velocity_.begin() + slice_vec_idx_ + 1);
  }
  void slice_curvature(int slice_vec_idx_) {
    curvature_ = std::vector<double>(curvature_.begin(),
                                     curvature_.begin() + slice_vec_idx_ + 1);
  }

  const std::vector<double>& points_d() const {
    return points_d_;
  }
  ...
  const std::vector<double>& points_x() const {
    return points_x_;
  }
  ...
  const std::vector<double>& delta_velocity() const {
    return delta_velocity_;
  }

  void set_cost_d(double cost_d) {
    cost_d_ = cost_d;
  }
  void set_cost_s(double cost_s) {
    cost_s_ = cost_s;
  }
  void set_cost_total(double cost_total) {
    cost_total_ = cost_total;
  }
  void set_blocking_flg(bool blk_flg) {
    blocking_flg_ = blk_flg;
  }
  void set_blocking_idx(int blk_idx) {
    blocking_idx_ = blk_idx;
  }
  ...
  bool blocking_flg() const {
    return blocking_flg_;
  }
  int blocking_idx() const {
    return blocking_idx_;
  }

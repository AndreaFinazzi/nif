...
def parse_configs():
    parser = argparse.ArgumentParser(description='The pcd to osm converter')
    ...
    if(configs.optional_dir == ''):
        ...
        root_path = './include/pcd/' + configs.optional_dir

    pcd_path = os.path.join(root_path, configs.filename) # input 
    osm_file_name = pcd_path[:-3] + "osm" 

    pc = pypcd.PointCloud.from_path(pcd_path)

    print("===========" * 5)
    print("INPUT : " , pcd_path)
    print("OUTPUT : " , osm_file_name)
    print(pc)

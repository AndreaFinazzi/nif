    ...
    nav_msgs::msg::Path &path_in_global_)
{
  ...
  for (int i = 0; i < path_in_global_.poses.size(); i++)
  {
    geometry_msgs::msg::PoseStamped point_in_body;
    point_in_body.pose.position.x =
        cos(-1 * current_yaw_rad) * (path_in_global_.poses[i].pose.position.x -
                                     current_pose_.pose.pose.position.x) -
        sin(-1 * current_yaw_rad) * (path_in_global_.poses[i].pose.position.y -
                                     current_pose_.pose.pose.position.y);
    point_in_body.pose.position.y =
        sin(-1 * current_yaw_rad) * (path_in_global_.poses[i].pose.position.x -
                                     current_pose_.pose.pose.position.x) +
        cos(-1 * current_yaw_rad) * (path_in_global_.poses[i].pose.position.y -
                                     current_pose_.pose.pose.position.y);
    point_in_body.pose.position.z =
        path_in_global_.poses[i].pose.position.z - current_pose_.pose.pose.position.z;

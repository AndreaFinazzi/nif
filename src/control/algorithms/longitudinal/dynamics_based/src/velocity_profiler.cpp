...
#include "dynamics_based_longi_control_algo/low_pass_filter.h"
...
namespace nif {
    namespace control {
        namespace longitudinal {
            ...
            class TireDynamicsVelProfiler {
            public:
                explicit TireDynamicsVelProfiler(const string &config_file_path_);
                ...
                const tireDynVelProfileParam &getParams() const;

                void setParams(const tireDynVelProfileParam &params);

                void setGlobalRefPath(nav_msgs::msg::Path::SharedPtr path_ptr);

                void setImuData(sensor_msgs::msg::Imu::SharedPtr imu_data_ptr);

                void setCurSteer(raptor_dbw_msgs::msg::SteeringReport::SharedPtr cur_steer_data_ptr);
...
//! Update Rate (in Hz)
    ...
    public:
        VelocityPlannerNode() : Node("VelocityPlannerNode") {
            // Publishers
            ...
            steering_cmd_sub_ = this->create_subscription<std_msgs::msg::Float32>(
                    ...
                    std::bind(&VelocityPlannerNode::steerCmdCallback, this,
                              std::placeholders::_1));
        ...
        void velocityPlanning() {
            ...
            bool valid_path = m_current_path.poses.size() > 0 &&
            ...
            if (m_vel_plan_enabled) {
                ...
                if (valid_path && valid_odom) {
                    ...
                    // double kappa = atan(m_current_steer_cmd) / m_L;
                    // double kappa = atan(m_current_steer) / m_L;
                    ...
                    // Get Lateral Tire Force Limit considering Tire load transfer.
                    auto tire_data =
                            m_tire_load_manager.Compute4WheelLateralForceLimit(a_lon, a_lat);
                    ...
                    double a_lat_max = 1 / m_total *
                                       (FyR + FyF * cos(m_current_steer) -
                                        m_total * current_velocity * yaw_rate);
                    ...
                    if (abs(kappa) <= m_CURVATURE_MINIMUM) {
                        ...
                        desired_velocity = sqrt(abs(a_lat_max) / kappa);
                        desired_velocity = std::min(desired_velocity, m_max_vel);
                        std::cout << "a_lat_max       : " << a_lat_max;
                        std::cout << "\na_lat           : " << a_lat;
                        std::cout << "\nm_current_steer : " << m_current_steer;
                        std::cout << "\nyaw_rate        : " << yaw_rate;
                        std::cout << "\nkappa           : " << kappa << std::endl;
                    }
                    // Decrease desired velocity w.r.t. lateral error
                    // double lat_error_vel_ratio = std::min(abs(m_error_y), m_ERROR_Y_MAX) /
                    //                      m_ERROR_Y_MAX;          // [0.0~1.0] ratio
                    double lat_error_vel_ratio = std::min(abs(m_error_y_lpf), m_ERROR_Y_MAX) /
                                                 m_ERROR_Y_MAX;          // [0.0~1.0] ratio
                    double lat_error_vel_neg_gain = 1.0 - 0.5 * lat_error_vel_ratio; // [1.0~0.5] gain

                    // TODO : double check that lat_error_vel_neg_gain is smaller than 1.0
                    lat_error_vel_neg_gain = std::clamp(lat_error_vel_neg_gain, 0, 1.0);

                    desired_velocity = lat_error_vel_neg_gain * desired_velocity;

                    // Publish diagnose message
                    publishDiagnostic(kappa, abs(a_lat_max), lat_error_vel_neg_gain);
            ...
            std::cout << "\ncurrent velocity : " << current_velocity << std::endl;
        }
        ...
        double computeCurvature(geometry_msgs::msg::PoseStamped pose_i_prev,
            ...
            double curvature = m_CURVATURE_MINIMUM;
        ...
        void publishDiagnostic(double curvature, double a_lat_max,
                               double error_gain) {
        ...
        void velocityCallback(
            ...
            m_current_speed_ms = (msg->rear_left + msg->rear_right) * 0.5 * kph2mps;
        }
        ...
        steerCallback(const raptor_dbw_msgs::msg::SteeringReport::SharedPtr msg) {
            m_current_steer = msg->steering_wheel_angle * deg2rad /
                              m_steer_ratio; // tire angle [deg --> rad]
        }

        void steerCmdCallback(const std_msgs::msg::Float32::SharedPtr msg) {
            m_current_steer_cmd = msg->data * deg2rad; // tire angle [deg --> rad]
        }
    ...
    private:
        ...
        TireManager m_tire_load_manager;
        //! Low pass filter
        ...
        //! Vehicle Params
        double m_L = m_tire_load_manager.L;                     // wheelbase
        double m_total = m_tire_load_manager.m_total;           // total mass [kg]
        double m_steer_ratio = m_tire_load_manager.steer_ratio; // steer ratio
        ...
        double m_current_steer_cmd;    // [rad]
...
int main(int argc, char **argv) {
    ...
    rclcpp::spin(std::make_shared<velocity_planner::VelocityPlannerNode>());
    rclcpp::shutdown();
    return 0;
}
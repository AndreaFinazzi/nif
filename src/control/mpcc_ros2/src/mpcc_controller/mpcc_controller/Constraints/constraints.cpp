...
OneDConstraint Constraints::getTrackConstraints(const ArcLengthSpline &track,
  ...
  // Compute bounds
  const double track_constraint_lower =
      tan_center(0) * pos_inner(0) + tan_center(1) * pos_inner(1);
  const double track_constraint_upper =
      tan_center(0) * pos_outer(0) + tan_center(1) * pos_outer(1);
...
OneDConstraint Constraints::getTrackConstraints(const ArcLengthSpline &track,
                                                const ArcLengthSpline &track_i,
                                                const ArcLengthSpline &track_o,
                                                const State &x) {
  // given arc length s and the track -> compute linearized track constraints
  const double s = x.s;
  ...
  // const Eigen::Vector2d pos_center = track.getClosestPostion(x, track);
  const Eigen::Vector2d d_center = track.getDerivative(s);
...
OneDConstraint Constraints::getTireConstraintRear(const State &x) const {
  ...
  //    const double tireCon0 = std::sqrt(std::pow(param_.e_long*f_rear.F_x,2) +
  //    std::pow(f_rear.F_y,2)); //zero order term
  const double tireCon0 =
      std::pow(param_.e_long * f_rear.F_x / f_normal.F_N_rear, 2) +
      std::pow(f_rear.F_y / f_normal.F_N_rear, 2); // zero order term
  const double maxForce =
      std::pow(param_.e_eps * param_.Dr / f_normal.F_N_rear,
               2); // param_.e_eps*param_.Dr;// //max allowed force
  ...
  const double tire_constraint_upper =
      maxForce + C_tire_constraint * x_vec - tireCon0;
...
C_i_MPC Constraints::getTireConstraintRearJac(const State &x) const {
  ...
  //    const double TC = 2.0*std::sqrt(std::pow(param_.e_long*f_rear.F_x,2) +
  //    std::pow(f_rear.F_y,2));
  //
  //    // Tire constraint derivatives
  //    // TC = (param_.E_long*Frx)^2 + Fry^2
  //    const double dTC_dvx = (2.0*param_.e_long*f_rear.F_x*df_rear.dF_x_vx
  //    + 2.0*f_rear.F_y*df_rear.dF_y_vx)/TC; const double dTC_dvy =
  //    (2.0*f_rear.F_y*df_rear.dF_y_vy)/TC; const double dTC_dr  =
  //    (2.0*f_rear.F_y*df_rear.dF_y_r)/TC; const double dTC_dD  =
  //    (2.0*param_.e_long*f_rear.F_x*df_rear.dF_x_D)/TC;

  const double TC =
      std::pow(param_.e_long * f_rear.F_x / f_normal.F_N_rear, 2) +
      std::pow(f_rear.F_y / f_normal.F_N_rear, 2);

  // Tire constraint derivatives
  // TC = (param_.e_long*Frx)^2 + Fry^2
  const double dTC_dvx =
      (2.0 * param_.e_long * f_rear.F_x / f_normal.F_N_rear * param_.e_long *
           df_rear.dF_x_vx / f_normal.F_N_rear +
       2.0 * f_rear.F_y / f_normal.F_N_rear * df_rear.dF_y_vx /
           f_normal.F_N_rear);
  const double dTC_dvy = (2.0 * f_rear.F_y / f_normal.F_N_rear *
                          df_rear.dF_y_vy / f_normal.F_N_rear);
  const double dTC_dr = (2.0 * f_rear.F_y / f_normal.F_N_rear * df_rear.dF_y_r /
                         f_normal.F_N_rear);
  const double dTC_dD = (2.0 * param_.e_long * f_rear.F_x / f_normal.F_N_rear *
                         param_.e_long * df_rear.dF_x_D / f_normal.F_N_rear);
...
OneDConstraint Constraints::getAlphaConstraintFront(const State &x) const {
  ...
  const double alpha_constraint_lower =
      -param_.max_alpha - alpha_f + C_alpha_constraint * x_vec;
  const double alpha_constraint_upper =
      param_.max_alpha - alpha_f + C_alpha_constraint * x_vec;
...
C_i_MPC Constraints::getAlphaConstraintFrontJac(const State &x) const {
  ...
  const double dalpha_f_dvx =
      (vy + r * param_.lf) /
      (std::pow(vy + r * param_.lf, 2) + std::pow(vx, 2));
  const double dalpha_f_dvy =
      -vx / (std::pow(vy + r * param_.lf, 2) + std::pow(vx, 2));
  const double dalpha_f_dr =
      -(vx * param_.lf) / (std::pow(vy + r * param_.lf, 2) + std::pow(vx, 2));
...
ConstrainsMatrix Constraints::getConstraints(const ArcLengthSpline &track,
  ...
  // TODO consider the zero order term directly in the functions construdcing
  // the constraints
  dl_constrains_matrix =
      dl_constrains_matrix - C_constrains_matrix * stateToVector(x);
  du_constrains_matrix =
      du_constrains_matrix - C_constrains_matrix * stateToVector(x);
...
ConstrainsMatrix Constraints::getConstraints(const ArcLengthSpline &track,
  ...
  OneDConstraint track_constraints =
      getTrackConstraints(track, track_i, track_o, x);
  const OneDConstraint tire_constraints_rear = getTireConstraintRear(x);
  const OneDConstraint alpha_constraints_front = getAlphaConstraintFront(x);

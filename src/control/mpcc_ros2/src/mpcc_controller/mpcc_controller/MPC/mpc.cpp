...
namespace mpcc
{
  ...
  void MPC::setTrack(const nav_msgs::msg::Path &planned_path)
  {
    if (planned_path.poses.size() != 0)
    {
      ...
      X_i(planned_path.poses.size() - 1) = planned_path.poses[planned_path.poses.size() - 1].pose.position.x - r * sin(slope + 0.000001);
      Y_i(planned_path.poses.size() - 1) = planned_path.poses[planned_path.poses.size() - 1].pose.position.y + r * cos(slope + 0.000001);
      X_o(planned_path.poses.size() - 1) = planned_path.poses[planned_path.poses.size() - 1].pose.position.x + r * sin(slope + 0.000001);
      Y_o(planned_path.poses.size() - 1) = planned_path.poses[planned_path.poses.size() - 1].pose.position.y - r * cos(slope + 0.000001);

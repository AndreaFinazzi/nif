...
:Ts_(1.0)
{
    std::cout << "(model) default constructor, not everything is initialized properly" << std::endl;
}

Model::Model(double Ts,const PathToJson &path)
:Ts_(Ts),param_(Param(path.param_path)),param_o_(Param(path.param_path))
{
    std::cout << "(model) Openning params at " << path.param_path << std::endl;
}
...
// Add getForceBack
TireForces Model::getForceBank(const State &x) const
{
    const double F_y = param_.m * param_.g * std::sin(x.B) * std::cos(center_yaw_error_);
    const double F_x = param_.m * param_.g * std::sin(x.B) * std::sin(center_yaw_error_);
...
NormalForces Model::getForceNormal(const State &x) const
{
    ...
    const double f_n_front = param_.lr/(param_.lf + param_.lr)*param_.m*param_.g*std::cos(x.B);
    const double f_n_rear = param_.lf/(param_.lf + param_.lr)*param_.m*param_.g*std::cos(x.B);
...
StateVector Model::getF(const State &x,const Input &u) const
{
    ...
    const TireForces tire_forces_bank  = getForceBank(x);
    ...
    f(2) = r;
    f(3) = 1.0/param_.m*(tire_forces_rear.F_x + friction_force - tire_forces_front.F_y*std::sin(delta) + param_.m*vy*r) + 1.0/param_.m*tire_forces_bank.F_x;
    f(4) = 1.0/param_.m*(tire_forces_rear.F_y + tire_forces_front.F_y*std::cos(delta) - param_.m*vx*r) + 1.0/param_.m*tire_forces_bank.F_y;

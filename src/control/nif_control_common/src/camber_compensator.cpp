#include "camber_compensator.hpp"

using namespace nif::control;

template <typename T> int sgn(T val) { return (T(0) < val) - (val < T(0)); }

CamberCompensator::CamberCompensator(CAMBERCOMPESATORMODE &mode_ = 0,
...
double CamberCompensator::getCamberCompensation() {
  ...
  if (abs(m_compensated_result) > abs(m_max_compensate_mag)) {
    ...
    m_compensated_result =
        m_sign_multiplier * m_max_compensate_mag * sng(m_compensated_result);
  } else {

...
protected:
  ...
  double getReferenceTrajLastPointDistance() const;
...
private:
   ...
   */
  double reference_trajectory_last_point_distance;
  ...
  rclcpp::Subscription<nif::common::msgs::Path>::SharedPtr reference_path_sub;
  rclcpp::Subscription<nif::common::msgs::ControlCmd>::SharedPtr
      control_cmd_prev_sub;
  rclcpp::Subscription<std_msgs::msg::Float32>::SharedPtr desired_velocity_sub;
  ...
  void
  referenceTrajectoryCallback(nif::common::msgs::Trajectory::SharedPtr msg);
  ...
  /**
   * slove() is called at each time-step and expects the next control command to
   * be returned. The returned message is automatically stamped by
   * IControllerNode, to avoid empty stamp errors.
   *

#include "nif_control_joint_lqr_nodes/utils/path_util.h"
...
void PathUtil::setConverter(GeodeticConverter &converter) {
  converter_ = converter;
}

void PathUtil::loadPath(std::string csv_path, std::string frame_id,
                        bool transform_to_ltp) {
  if (transform_to_ltp && !converter_.isInitialised()) {
    throw std::runtime_error("Unable to transform to ltp when converter_ is "
...
nav_msgs::msg::Path PathUtil::wrapPathTo(geometry_msgs::msg::Pose pose,
                                         int pose_limit) {
  ...
  if (pose_limit > 0)
    pose_count = static_cast<unsigned int>(pose_limit);
  if (pose_limit < 0)
    pose_count -= 100;
...
nav_msgs::msg::Path &PathUtil::getPath() { return path_; }

unsigned int PathUtil::getClosestPoint(geometry_msgs::msg::Pose pose) {
...
PathUtil::computePointOrientation(nav_msgs::msg::Path &path, unsigned int idx) {
  ...
  yaw = std::atan2(path.poses[idx_up].pose.position.y -
                       path.poses[idx_down].pose.position.y,
                   path.poses[idx_up].pose.position.x -
                       path.poses[idx_down].pose.position.x);

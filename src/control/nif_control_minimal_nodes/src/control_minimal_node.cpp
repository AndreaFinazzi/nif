...
: IControllerNode(node_name)
{
  ...
  subPath_ = this->create_subscription<nav_msgs::msg::Path>(
      "target_path", 1, std::bind(&ControlMinimalNode::receivePath, this, std::placeholders::_1));
  subVelocity_ = this->create_subscription<novatel_gps_msgs::msg::NovatelVelocity>(
      "novatel_bottom/bestvel", 1, std::bind(&ControlMinimalNode::receiveVelocity, this, std::placeholders::_1));
  subJoySteering_ = this->create_subscription<std_msgs::msg::Float32>(
      "/joystick/steering_cmd_not_used", 1, std::bind(&ControlMinimalNode::receiveJoySteer, this, std::placeholders::_1));

  // Create Timer
//  control_timer_ = this->create_wall_timer(
//      std::chrono::milliseconds(10), std::bind(&ControlMinimalNode::controlCallback, this));
...
int ControlMinimalNode::findLookaheadIndex(std::vector<geometry_msgs::msg::PoseStamped> refPath, double desLookaheadValue)
{
...
void ControlMinimalNode::receiveVelocity(const novatel_gps_msgs::msg::NovatelVelocity::SharedPtr msg)
{
  this->speed_ = msg->horizontal_speed;
}

void ControlMinimalNode::receiveJoySteer(const std_msgs::msg::Float32::SharedPtr msg)
{

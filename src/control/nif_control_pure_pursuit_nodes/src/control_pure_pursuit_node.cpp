...
ControlPurePursuitNode::ControlPurePursuitNode(const std::string &node_name)
  ...
  try {
    ...
    RCLCPP_ERROR(
        this->get_logger(),
        "Failed to declare or get parameters. Setting defaults values.");
...
/**
 * solve() is called every time-step, and its result is published as output of
 * the controller.
 *
 * @return reference to ControlCmd
 */
nif::common::msgs::ControlCmd::SharedPtr ControlPurePursuitNode::solve() {
  // TODO update to comply with IControllerNode last version (embedded path
  // subscriber and has...() methods)
  if ((this->getReferenceTrajectory() &&
    ...
    m_lookahead_pt_in_global =
        m_pure_pursuit_handler_ptr->getLookaheadPointInGlobal();

...
void nif::control::ControlSafetyLayerNode::controlCallback(
  ...
  if ((this->now() - msg->header.stamp) < this->getGclockPeriodDuration())
...
void nif::control::ControlSafetyLayerNode::afterSystemStatusCallback() {
  ...
  if (this->getSystemStatus()
          .health_status.communication_failure || // too conservative
      (this->lat_autonomy_enabled &&
...
void nif::control::ControlSafetyLayerNode::controlOverrideCallback(
  ...
  this->emergency_manual =
      (this->override_control_cmd.braking_control_cmd.data >=
       this->override_brake_deadband);
}
...
void nif::control::ControlSafetyLayerNode::run() {
  ...
  if (this->emergency_lane_enabled || this->emergency_buffer_empty ||
      this->emergency_manual || !this->hasSystemStatus()) {
    // STOP THE CAR ASAP, IN A SAFE MANNER (set_speed_mps=0.0, brake_cmd based
    // on PID, accel_cmd=0)
    ...
    //        TODO this steering command should derive from perception-based
    //        controller
    ...
    } else if (this->emergency_wall_distance && this->has_perception_steering &&
               this->now() - this->perception_steering_last_update <
                   rclcpp::Duration(1, 0)) {
    ...
    } else if (!this->emergency_buffer_empty && !is_buffer_empty &&
               !this->getSystemStatus().health_status.localization_failure) {

      auto top_control_cmd = std::move(*this->control_buffer.top());
      this->control_cmd.steering_control_cmd =
          top_control_cmd.steering_control_cmd;
    ...
    if (this->emergency_buffer_empty || !this->hasSystemStatus()) {
    ...
    if (this->emergency_buffer_empty && !lat_autonomy_enabled &&
    ...
    try {
      if (lat_autonomy_enabled || long_autonomy_enabled) {
        if (!is_buffer_empty) {
          ...
          if (long_autonomy_enabled) {
            this->control_cmd.desired_accel_cmd =
                top_control_cmd.desired_accel_cmd;
          } else {
          ...
          if (this->buffer_empty_counter >=
              this->buffer_empty_counter_threshold) {
      ...
      // if (this->override_control_cmd.braking_control_cmd.data >
      // this->override_brake_deadband)
      // {
      ...
      if (long_autonomy_enabled) {
        ...
        // Publish joystck commands if long_autonomy_enable is false and no
        // braking is commanded
      } else {
        this->publishBrakingCmd(this->override_control_cmd.braking_control_cmd);
        this->publishAcceleratorCmd(
            this->override_control_cmd.accelerator_control_cmd);
      }
      ...
      RCLCPP_ERROR(this->get_logger(),
                   "ControlSafetyLayerNode caught an exception, enabling "
...
bool nif::control::ControlSafetyLayerNode::publishDesiredAcceleration(
    std_msgs::msg::Float32 msg) const {
  //  TODO implement safety checks

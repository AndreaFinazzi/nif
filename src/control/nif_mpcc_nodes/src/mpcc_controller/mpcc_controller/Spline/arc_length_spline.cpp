...
Eigen::VectorXd
ArcLengthSpline::compArcLength(const Eigen::VectorXd &X_in,
...
RawPath
ArcLengthSpline::outlierRemoval(const Eigen::VectorXd &X_original,
...
Eigen::Vector2d
ArcLengthSpline::getClosestPostion(const State x,
                                   const ArcLengthSpline &track) const {
  Eigen::Vector2d pos;
  double closest_dist_sq = 9999. * 9999.;
  for (int i = 0; i < track.path_data_.X.size(); i++) {
    double track_x = track.path_data_.X(i);
    double track_y = track.path_data_.Y(i);
...
double ArcLengthSpline::porjectOnSpline(const State &x) const {
  ...
  // std::cout << "pos x, y : " << pos(0) << " " << pos(1) << " " << "pos_path
  // x, y : " << pos_path(0) << " " << pos_path(1) << std::endl;
  ...
  for (int i = 0; i < 20; i++) {
    ...
    if (std::abs(s_old - s_opt) <= 1e-5) {
      // std::cout << "curr s : " << x.s << " " << "s_opt : " << s_opt << " " <<
      // "s_guess : " << s_guess << std::endl;
      return s_opt;
    }
    s_old = s_opt;
  }
...
double ArcLengthSpline::projectOnSpline(const State &x) {
  Eigen::Vector2d pos;
  double closest_dist_sq = 9999. * 9999.;
  int closest_index = 0;
  double progress = 0;
  ...
  // find closest (x,y), index and progress
  for (int i = 1; i < path_data_.X.size(); i++) {
    double track_x = path_data_.X(i);
    double track_y = path_data_.Y(i);
    double dist_sq = pow(track_x - x.X, 2.) + pow(track_y - x.Y, 2.);

    progress += sqrt(pow(path_data_.X(i) - path_data_.X(i - 1), 2.) +
                     pow(path_data_.Y(i) - path_data_.Y(i - 1), 2.));

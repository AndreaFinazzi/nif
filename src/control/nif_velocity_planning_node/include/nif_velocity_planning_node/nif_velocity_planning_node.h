...
//! Update Rate (in Hz)
        ...
        public:
            VelocityPlannerNode(
                    ...
                    : IBaseSynchronizedNode(node_name, common::NodeType::CONTROL, timer_period, options) {
                ...
                if (m_odometry_timeout_sec <= 0. || m_path_timeout_sec <= 0.) {
                    RCLCPP_ERROR(this->get_logger(),
                                 "path and ego_odometry timeouts must be greater than zero. Got m_odometry_timeout_sec: %f; m_path_timeout_sec: %f",
                                 m_odometry_timeout_sec, m_path_timeout_sec);
                    throw std::range_error("Parameter out of range.");
                }

                m_odometry_timeout = rclcpp::Duration(0, m_odometry_timeout_sec * 1000000000);
        ...
        private:
            ...
            void run() override {
                ...
                bool valid_path = has_path &&
                                  !m_current_path.poses.empty() && // TODO too loose, require a minimum length.
            ...
            void afterSystemStatusCallback() override {
                ...
                if (this->hasSystemStatus()) {
                    this->m_max_vel_mps = this->getSystemStatus().mission_status.max_velocity_mps;
                }
            }
                ...
                for (const auto &param : vector) {
                    ...
                    if (param.get_name() == "des_vel_mps") {
                        ...
                        if (param.get_type() == rclcpp::ParameterType::PARAMETER_BOOL &&
                            ...
                            result.successful = false;
                            result.reason = "vel_plan_enabled must be a boolean, and the current speed must be lower than 0.1 m/s .";
                        }
                    }
                }
                return result;

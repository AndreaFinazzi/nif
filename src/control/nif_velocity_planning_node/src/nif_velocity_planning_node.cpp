...
public:
  ...
  void velocityPlanning() {
    ...
    if (m_vel_plan_enabled) {
      ...
      if (valid_path && valid_odom) {
        ...
        // Publish maximum desired velocity
  ...
  void steerCmdCallback(const std_msgs::msg::Float32::SharedPtr msg) {
    m_current_steer_cmd =
        msg->data * nif::common::constants::RAD2DEG; // tire angle [deg --> rad]
  }
...
private:
  ...
  //! Vehicle Params
  double m_L = m_tire_manager.L;                     // wheelbase
  double m_total = m_tire_manager.m_total;           // total mass [kg]
  double m_steer_ratio = m_tire_manager.steer_ratio; // steer ratio

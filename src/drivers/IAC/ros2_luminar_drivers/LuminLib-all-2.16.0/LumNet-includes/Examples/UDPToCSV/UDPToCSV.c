 ...
 *
 *   $ Or:
 *     Run CMake to generate platform specific project for your compiler of
 * choice
 *
 * Usage:
...
#define MAX_PORTNO (65535)
  ...
  printf("TIMESTAMP(SEC), TIMESTAMP(USEC), SCAN COUNTERS, SCAN COUNT, "
         "INTERLACE INDEX, CHECKPOINT, TRACK NUMBER, TRACK CHECKPOINT, SSI, "
    ...
    for (uint16_t rayIdx = 0; rayIdx < rayIndexLength; rayIdx++) {
      for (uint8_t returnIdx = 0; returnIdx < rayIndex[rayIdx].num_returns;
        ...
        printf("%u, "                       // TIMESTAMP(SEC),
               ...
               LumNet_RayHeader_GetScanCount(preamble->scan_counters),
               LumNet_RayHeader_GetInterlaceIndex(preamble->scan_counters),
               preamble->checkpoint, // CHECKPOINT
               LumNet_RayHeader_GetTrackNumber(preamble->checkpoint),
               LumNet_RayHeader_GetTrackCheckpoint(preamble->checkpoint),
               LumNet_RayHeader_GetSSI(&ray->rayHeader),         // SSI
               LumNet_RayHeader_GetEyeIndex(&ray->rayHeader, 1), // EYE
               (double) LumNet_SignedFixedToDegrees(
                   LumNet_RayHeader_GetAngleAzimuth(&ray->rayHeader)),
               (double) LumNet_SignedFixedToDegrees(
                   LumNet_RayHeader_GetAngleElevation(&ray->rayHeader)),
               returnIdx,
               (double) LumNet_RangeFixedToFloat(

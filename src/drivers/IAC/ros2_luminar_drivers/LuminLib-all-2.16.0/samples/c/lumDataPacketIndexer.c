...
enum LumDataPacketIndexer_returnCode LumDataPacketIndexer_Index(
    char const *const packet,
    uint32_t const packetNumBytes,
    struct LumNet_LidarDataPreamble const **const preamble,
    struct LumDataPacketIndexer_rayIndexEntry *const rayIndexPool,
    uint16_t const rayIndexPoolMaxLength,
    uint16_t *const rayIndexLength )
{
    ...
    *preamble = (struct LumNet_LidarDataPreamble *)( packet + packetIndex );

    const struct LumNet_Version parserSemver = LumNet_GetLidarDataProtocolSemVer();
    if ( !LumNet_VersionIsCompatible( parserSemver, ( *preamble )->semVersionedHeader.semver ) )
    {
#if LUM_DATA_PACKET_INDEXER_PRINT_DEBUG == true
        fprintf( stderr,
            ...
            *rayIndexLength < rayIndexPoolMaxLength )
    {
        ...
        rayIndexPool[*rayIndexLength].ray =
            (struct LumNet_Ray *)( packet + packetIndex );
        ...
        while ( packetIndex < packetNumBytes &&
                LumNet_IsWordRayReturn( (uint32_t *)( packet + packetIndex ) ) )
        {
...
static inline bool LumNet_IsWordRayReturn( uint32_t const *const word )
{
    return LumNet_RayReturn_GetReturnBit( (LumNet_RayReturn *)word ) == 0;
}

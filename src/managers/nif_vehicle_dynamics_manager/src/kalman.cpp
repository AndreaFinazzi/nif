...
void KalmanFilter::predict(float dt) { predict(dt, 0, nmodels); }
...
float KalmanFilter::get(unsigned int istate) { return pmodels[istate].x[0]; }
...
float &Vec2::operator[](int index) { return index == 0 ? x : y; }
...
Mat2x2 Mat2x2::transposed() { return Mat2x2(m00, m10, m01, m11); }

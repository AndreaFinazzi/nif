...
nif::managers::WaypointManagerMissionSelectiveNode::WaypointManagerMissionSelectiveNode(
  ...
  try {
    ...
    package_share_directory = ament_index_cpp::get_package_share_directory(
        "nif_waypoint_manager_nodes_selective");
  ...
  m_timer = this->create_wall_timer(
      10ms, std::bind(&WaypointManagerMissionSelectiveNode::timerCallback, this));
  ...
  m_obstacle_subscriber = this->create_subscription<visualization_msgs::msg::MarkerArray>(
          "/joystick/command", nif::common::constants::QOS_PLANNING,
          std::bind(&WaypointManagerMissionSelectiveNode::obstacleCallback, this,
                    std::placeholders::_1));
...
void nif::managers::WaypointManagerMissionSelectiveNode::obstacleCallback(const visualization_msgs::msg::MarkerArray::SharedPtr obj_array) {
  this->wpt_manager->setObstacle(*obj_array);
}

void nif::managers::WaypointManagerMissionSelectiveNode::timerCallback() try {
  //  RCLCPP_DEBUG(this->get_logger(), "WaypointManagerMissionSelectiveNode timer
  //  callback");

...
// Constructor
VisualToCostmap::VisualToCostmap() :
NUMBER_OF_POINTS(4),
...
Eigen::MatrixXd VisualToCostmap::makeRectanglePoints(const detection_msgs::BoundingBox& in_bboxes,
...
grid_map::Polygon VisualToCostmap::makePolygonFromBBox(const detection_msgs::BoundingBox& in_bboxes,
                                                       const double expand_rectangle_size)
{
...
geometry_msgs::Point VisualToCostmap::makeExpandedPoint(const geometry_msgs::Point& in_centroid,
...
void VisualToCostmap::setCostInPolygon(const grid_map::Polygon& polygon, const std::string& gridmap_layer_name,
...
grid_map::Matrix VisualToCostmap::makeCostmapFromBBoxes(const grid_map::GridMap& costmap,
                                                        const double expand_polygon_size,
                                                        const double size_of_expansion_kernel,
                                                        const detection_msgs::BoundingBoxArray::ConstPtr& in_bboxess)
{
  grid_map::GridMap objects_costmap = costmap;
  objects_costmap.add(VISUAL_COSTMAP_LAYER_, 0);
  ...
  for (const auto& object : in_bboxess->boxes)
  {
    ...
    setCostInPolygon(expanded_polygon, VISUAL_COSTMAP_LAYER_, 100, objects_costmap);
  }
  return objects_costmap[VISUAL_COSTMAP_LAYER_];
}

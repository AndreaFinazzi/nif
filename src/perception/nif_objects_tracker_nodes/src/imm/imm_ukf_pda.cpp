      ...
      init_(false), frame_count_(0)
{
    ...
    {
        throw std::runtime_error("static_num_history_thres is not properly settup.");
    }
    else
    {
...
void ImmUkfPda::setDetectionResult(const nif_msgs::msg::DetectedObjectArray &input)
{
...
nif_msgs::msg::DetectedObjectArray ImmUkfPda::getTrackedResult(nif_msgs::msg::DetectedObjectArray &input)
{
    ...
    nif_msgs::msg::DetectedObjectArray &detected_objects_output)
{
    ...
    for (auto &object : detected_objects_output.objects)
    {
        geometry_msgs::msg::Pose out_pose;
        out_pose.position.x =
            cos(-1 * current_yaw_rad) * (object.pose.position.x -
                                         ego_odom_.pose.pose.position.x) -
            sin(-1 * current_yaw_rad) * (object.pose.position.y -
                                         ego_odom_.pose.pose.position.y);
        out_pose.position.y =
            sin(-1 * current_yaw_rad) * (object.pose.position.x -
                                         ego_odom_.pose.pose.position.x) +
            cos(-1 * current_yaw_rad) * (object.pose.position.y -
                                         ego_odom_.pose.pose.position.y);
        out_pose.position.z =

...
public:
  ...
  std::vector<std::shared_ptr<FrenetPath>> calc_frenet_paths_multi_longi_body(
      double current_position_d, double current_position_s,
      double current_velocity_d, double current_velocity_s,
      double current_acceleration_d,
      std::shared_ptr<CubicSpliner2D> &cubic_spliner_2D, double min_t,
  ...
  CubicSpliner2DResult apply_cubic_spliner(
      std::vector<double> &points_x,
      std::vector<double> &points_y,
      double spline_interval);

  CubicSpliner2DResult_w_progress
  apply_cubic_spliner_from_nav_path(
      nav_msgs::msg::Path &path_,
      double spline_interval);

  nif_msgs::msg::DynamicTrajectory
  convert_path_to_traj_curv(
      nav_msgs::msg::Path &path_,
  ...
  CubicSpliner2DResult
  apply_cubic_spliner_w_current_pose(
      std::vector<double> &points_x,
      std::vector<double> &points_y,

...
public:
  ...
  calculate_optimal_frenet_path(
      ...
      std::vector<std::tuple<double, double>> &obstacles, double min_t,
      double max_t, double dt);

  std::tuple<std::shared_ptr<FrenetPath>,
             std::vector<std::shared_ptr<FrenetPath>>>
  calc_frenet_paths(double current_position_d, double current_position_s,
                    double current_velocity_d, double current_velocity_s,
                    double current_acceleration_d,
                    std::shared_ptr<CubicSpliner2D> &cubic_spliner_2D,
  ...
  nif_msgs::msg::DynamicTrajectory
  convert_path_to_traj_curv(nav_msgs::msg::Path &path_, int current_idx,
                            double max_lateral_acceleration_,
                            double spline_interval_);
...
protected:
  std::vector<std::shared_ptr<FrenetPath>>
  generate_frenet_paths(double current_position_d, double current_position_s,
                        double current_velocity_d, double current_velocity_s,
                        double current_acceleration_d, double min_t,
                        double max_t, double dt);

  std::vector<std::shared_ptr<FrenetPath>>
  generate_frenet_paths_v2(double current_position_d, double current_position_s,
                           double current_velocity_d, double current_velocity_s,
                           double current_acceleration_d, double min_t,

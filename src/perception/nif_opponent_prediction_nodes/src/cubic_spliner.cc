#include "nif_opponent_prediction_nodes/cubic_spliner.h"
#include <assert.h>
    ...
    : points_x_(points_x), points_y_(points_y)
{
  ...
  for (auto itr = points_x_.begin(); itr != points_x_.end(); itr++)
  {
    ...
    double b =
        (coefficient_a_[i + 1] - coefficient_a_[i]) / delta_x[i] -
        delta_x[i] * (coefficient_c_[i + 1] + 2.0 * coefficient_c_[i]) / 3;
    double d = (coefficient_c_[i + 1] - coefficient_c_[i]) / (3.0 * delta_x[i]);

    // std::cout << "coefficient_a_[i + 1] : " << coefficient_a_[i + 1]
    //           << std::endl;
    // std::cout << "delta_x[i] : " << delta_x[i] << std::endl;
    // std::cout << "coefficient_c_[i + 1] : " << coefficient_c_[i + 1]
    //           << std::endl;
...
double CubicSpliner::calculate_zeroth_derivative(double time)
{
  ...
  double zeroth_derivative =
      coefficient_a_[index] + coefficient_b_[index] * dx +
      coefficient_c_[index] * pow(dx, 2) + coefficient_d_[index] * pow(dx, 3);
...
double CubicSpliner::calculate_first_derivative(double time)
{
  ...
  double first_derivative = coefficient_b_[index] +
                            2 * coefficient_c_[index] * dx +
                            3 * coefficient_d_[index] * pow(dx, 2);

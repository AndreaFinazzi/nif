    ...
    : IBaseNode(node_name_, common::NodeType::PLANNING)
{
  ...
  std::cout << "Num of overtaking candidates : " << m_num_overtaking_candidates << std::endl;
  ...
  m_ego_sensor_gt_sub = this->create_subscription<nav_msgs::msg::Odometry>(
      "/sensor/odom_ground_truth", common::constants::QOS_EGO_ODOMETRY,
      std::bind(&DynamicPlannerNode::sensorGTCallback, this,
                std::placeholders::_1));
  m_keystrike_sub = this->create_subscription<std_msgs::msg::UInt8>(
      "/keystrike", 10,
...
void DynamicPlannerNode::timer_callback_imitative()
{
  ...
  if (m_previous_imitative_path_idx == cur_path_idx)
  {
    ...
    if (cur_idx_on_previous_path >= m_reset_wpt_idx)
    {
      ...
      m_reset_wpt_idx = 10000000000000000;
    }
  ...
  else
  {
    ...
    auto progressNcte = calcProgressNCTE(
        m_ego_odom.pose.pose,
        m_overtaking_candidates_path_vec[cur_path_idx]);

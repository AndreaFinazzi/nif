...
bool velocity_profiler::parseConfig_(std::string & config_file_path_)
{
  ...
  if (m_constraint_max_t < m_constraint_min_t) {
    throw std::runtime_error(
            "m_constraint_max_t can not be less than "
            "m_constraint_min_t. Check config file.");
  }
  if (m_constraint_max_accel < 0.0) {
    throw std::runtime_error(
            "m_constraint_max_accel can not be less than "
            "zero. Check config file.");
  }
  if (m_constraint_max_lat_accel < 0.0) {
    throw std::runtime_error(
            "m_constraint_max_lat_accel can not be less than "
            "zero. Check config file.");
  }
  if (m_constraint_max_deccel > 0.0) {
    throw std::runtime_error(
            "m_constraint_max_deccel can not be larger than "
            "zero. Check config file.");
  }
  if (m_constraint_max_vel < 0.0) {
    throw std::runtime_error(
            "m_constraint_max_vel can not be less than "
            "zero. Check config file.");
  }
  ...
  while (point_s < point_s_end) {
    ...
    if (point_s == 0.0) {
      ...
      out_traj.trajectory_timestamp_array.push_back(
        ...
        (spline_interval_ /
        (std::max(
          double(out_traj.trajectory_velocity.back()),
          MIN_SPEED_MPS))));
    ...
    if (!oppo_predicted_path_.trajectory_path.poses.empty()) {
      ...
      if (!use_sat_) {
        ...
        if (nif::planning::sat::separating_axis_intersect(
            bound_ego,
            bound_oppo) == true)
        {
          has_collision = true;
    ...
    while (point_s < point_s_end) {
      ...
      if (point_s == 0.0) {
        ...
        auto naive_cur_gap = std::max(
          sqrt(
            pow(predictided_oppo_pose.pose.position.x - point_x, 2) +
        ...
        auto acc_desired_accel =
          ...
          (1 -
          pow(
            (out_traj.trajectory_velocity.back() / m_acc_config_v_desired),
            m_acc_config_delta) -

...
class ImitativeModel(nn.Module):
        ...
        **context: torch.Tensor,
        ...
        for _ in range(num_steps):
            ...
            imitation_prior = torch.mean(
                log_prob - logabsdet
            )  # pylint: disable=no-member q(x|phi)
    ...
    def _region_based_planning(
        ...
        **context: torch.Tensor,
        ...
        x_best = x.clone()
        loss_best = (
            torch.ones((batch_size)).to(x.device) * 1000.0
        )  # pylint: disable=no-member
        ...
        for _ in range(num_steps):
            ...
            # Book-keeping
            x_best = torch.where(
                loss.view(-1, 1, 1) < loss_best.view(-1, 1, 1), x, x_best
            ).clone()
    ...
    def _region_goal(
        ...
        **hyperparams,
        ...
        log_qx = torch.max(
            q_T.log_prob(y[:, :2].unsqueeze(dim=1).repeat((1, segment.shape[1], 1))),
            dim=1,
        )[0]
        prec_mat = torch.inverse(cov_mat)

        unum = -torch.einsum(
            "bmd,bde,bme->bm", segment, prec_mat, start - mu_T[:, None, :]
        )
    ...
    def transform(
        self, sample: Mapping[str, types.Array]

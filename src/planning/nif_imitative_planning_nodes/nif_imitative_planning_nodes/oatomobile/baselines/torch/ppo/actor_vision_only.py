...
class CNNBase(NNBase):
    def __init__(self, input_shape, num_outputs, recurrent=False, hidden_size=512, include_depth=True):
        ...
        if self.include_depth:
            self._encoder = MobileNetV2(num_classes=hidden_size, in_channels=4)
        else:
            self._encoder = MobileNetV2(num_classes=hidden_size, in_channels=3)
    ...
    # def forward(self, **context: torch.Tensor) -> torch.Tensor:
        ...
        x = self._encoder(visual_features)

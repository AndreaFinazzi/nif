...
class CARLADataset(Dataset):
    ...
    def load_datum(
        ...
        Args:
          fname: The absolute path to the ".npz" or ".dill" datum.
        ...
        sample = dict()
        # CHANGE LOADING BASED ON DILL OR NPZ
        # with np.load(fname) as datum:
        with open(fname, 'rb') as f:
            datum = dill.load(f, encoding='latin1')
            for attr in modalities:
                dtype = np.uint8 if attr.endswith('_color') else np.float32
                # Fetches the value.
        ...
        if "costmap" in sample:
            sample["costmap"] = np.pad(sample["costmap"], ((0, 120-sample["costmap"].shape[0]), (0, 120-sample["costmap"].shape[1]), (0, 0)))
        ...
        if mode and "player_future" in sample:
            ...
            sample["mode"] = sample["mode"].astype(np.float32)
    ...
    def as_torch(
        ...
        class PyTorchDataset(torch.utils.data.Dataset):
            ...
            def __init__(
                ...
                # SWITCHED WITH DILL FROM NPZ
                self._dill_files = glob.glob(os.path.join(dataset_dir, "*.dill"))
                self._transform = transform
                self._mode = mode

            def __len__(self) -> int:
                """Returns the size of the dataset."""
                return len(self._dill_files)

...
class ACClientNode(rclpy.node.Node):
    def __init__(self, node_name: str, udp_client: SocketType) -> None:
        ...
        # Timers
        self.timer_recv = self.create_timer(
            TIMER_PERIOD_RECV_S, self.timer_recv_callback
        )
        ...
        self.pub_fake_aw_odom = self.create_publisher(
            Odometry, "/aw_localization/ekf/odom", rclpy.qos.qos_profile_sensor_data
        )
        self.pub_pt_report = self.create_publisher(
            PtReport, "/raptor_dbw_interface/pt_report", 20
        )
        self.pub_wheel_speed = self.create_publisher(
            WheelSpeedReport, "/raptor_dbw_interface/wheel_speed_report", 20
        )  # rclpy.qos.qos_profile_sensor_data)

        self.pub_dummy_localization_status = self.create_publisher(
            LocalizationStatus, "/aw_localization/ekf/status", 10
        )
        self.pub_dummy_raptor_diag = self.create_publisher(
            DiagnosticReport, "/raptor_dbw_interface/diag_report", 10
        )

        self.pub_car_count = self.create_publisher(
            UInt8, "/ac/car_count", qos_car_count
        )
        self.pub_oppo_markers = self.create_publisher(
            MarkerArray, "/ac/vis/cars", rclpy.qos.qos_profile_sensor_data
        )
    ...
    def parseState(self, state: dict):
        if self.is_first_call:
            ...
            for cid in range(state["carsCount"]):
                ...
                self.pubs_car_status.append(
                    self.create_publisher(
                        ACTelemetryCarStatus,
                        "/ac/car_status_" + str(cid),
                        rclpy.qos.qos_profile_sensor_data,
                    )
                )
                self.pubs_car_marker.append(
                    self.create_publisher(
                        Marker,
                        "/ac/car_marker_" + str(cid),
                        rclpy.qos.qos_profile_sensor_data,
                    )
                )
    ...
    def update_oppo_markers(self, state):
        ...
        for (cid, csWorldPosition) in enumerate(state["csWorldPosition"]):
            ...
            car_status.linear_velocity_vector.x = state["csLinearVelocityVector"][cid][
                0
            ]
            car_status.linear_velocity_vector.y = state["csLinearVelocityVector"][cid][
                1
            ]
            car_status.linear_velocity_vector.z = state["csLinearVelocityVector"][cid][
                2
            ]

            car_status.twist_local.linear.x = state["csLinearVelocityVectorLocal"][cid][
                0
            ]
            car_status.twist_local.linear.y = state["csLinearVelocityVectorLocal"][cid][
                1
            ]
            car_status.twist_local.linear.z = state["csLinearVelocityVectorLocal"][cid][
                2
            ]

            car_status.twist_local.angular.x = state["csAngularVelocityVectorLocal"][
                cid
            ][0]
            car_status.twist_local.angular.y = state["csAngularVelocityVectorLocal"][
                cid
            ][1]
            car_status.twist_local.angular.z = state["csAngularVelocityVectorLocal"][
                cid
            ][2]
            ...
            car_status.wheel_angular_speed.append(
                state["csWheelAngularVelocityVector"][cid][0]
            )
            car_status.wheel_angular_speed.append(
                state["csWheelAngularVelocityVector"][cid][1]
            )
            car_status.wheel_angular_speed.append(
                state["csWheelAngularVelocityVector"][cid][2]
            )
            car_status.wheel_angular_speed.append(
                state["csWheelAngularVelocityVector"][cid][3]
            )
        ...
        for sorted_idx in sorted_idx_list:
            ...
            if sorted_idx == 0:
                ...
                self.pub_odom_ground_truth.publish(self.ego_odom)
                self.pub_fake_aw_odom.publish(self.ego_odom)
                self.tf_broadcast(self.ego_odom)

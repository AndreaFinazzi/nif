...
class BagFileParser():
    def __init__(self, bag_file_path):
        ...
        self.topic_msg_message = {name_of: get_message(type_of) for id_of, name_of, type_of in topics_data if type_of == 'nav_msgs/msg/Odometry'}
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--ai-topic-prefix", type=str, default="/novatel_bottom/odom")
    ...
    for db3_file in db3_file_list:
        ...
        with open(filename, mode='w') as file:
            ...
            field = []
            for ai_idx, ai_topic_name in enumerate(ai_msgs_topic_name_list):
                ai_msgs = parser.get_messages(ai_topic_name)
                ai_pos_x = [ai_msgs[i][1].pose.pose.position.x for i in range(len(ai_msgs))]
                ai_pos_y = [ai_msgs[i][1].pose.pose.position.y for i in range(len(ai_msgs))]
                ...
                ai_pos_stamp = [ai_msgs[i][1].header.stamp for i in range(len(ai_msgs))]

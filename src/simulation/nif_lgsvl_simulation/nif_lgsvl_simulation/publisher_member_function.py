...
class Drive(Node):
    def __init__(self):
        ...
        self.namespace = ''
        self.sub_accel= self.create_subscription(Float32, self.namespace + '/raptor_dbw_interface/accelerator_pedal_cmd', self.callback_accel, 10)
        self.sub_steer = self.create_subscription(Float32, self.namespace + '/raptor_dbw_interface/steering_cmd', self.callback_steer, 10)
        self.sub_brake = self.create_subscription(Float32, self.namespace + '/raptor_dbw_interface/brake_cmd', self.callback_brake, 10)
        self.sub_gear = self.create_subscription(Int8, self.namespace + '/raptor_dbw_interface/gear_cmd', self.callback_gear, 10)


        self.control_pub = self.create_publisher(VehicleControlData, self.namespace + '/sensor/control', 10)

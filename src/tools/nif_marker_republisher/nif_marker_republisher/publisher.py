...
#     # 'RR' : [ac.getCarState(cid, nif_marker_republisher
...
class PubNode(rclpy.node.Node):
    def __init__(self, node_name: str, ) -> None:
        ...
        # subscriber
        self.predicted_traj_sub = self.create_subscription(Path, '/oppo/vis/prediction', self.oppo_predicted_path_callback, rclpy.qos.qos_profile_sensor_data)
        ...
        self.ego_xacro_pub = self.create_publisher(Marker, '/ego/xacro/marker', rclpy.qos.qos_profile_sensor_data)
        self.oppo_xacro_pub = self.create_publisher(Marker, '/oppo/xacro/marker', rclpy.qos.qos_profile_sensor_data)
    ...
    def oppo_predicted_path_callback(self, msg):

        print("oppo subed")

        oppo_marker = Marker()
        oppo_marker.header.frame_id = "odom"
        oppo_marker.id = 0
        oppo_marker.type = 10
        oppo_marker.mesh_resource = "package://il15_description/visual/il15.dae"
        oppo_marker.action = oppo_marker.ADD

        oppo_marker.pose.position.x = msg.poses[0].pose.position.x
        oppo_marker.pose.position.y = msg.poses[0].pose.position.y
        oppo_marker.pose.position.z = msg.poses[0].pose.position.z
        oppo_marker.pose.orientation.x = msg.poses[0].pose.orientation.x
        oppo_marker.pose.orientation.y = msg.poses[0].pose.orientation.y
        oppo_marker.pose.orientation.z = msg.poses[0].pose.orientation.z
        oppo_marker.pose.orientation.w = msg.poses[0].pose.orientation.w

        oppo_marker.lifetime = Duration(seconds=1, nanoseconds=20000000).to_msg()
        oppo_marker.scale.x = 1.0
        oppo_marker.scale.y = 1.0
        oppo_marker.scale.z = 1.0
        oppo_marker.color.a = 1.0
        oppo_marker.color.r = 0.4
        oppo_marker.color.g = 0.0
        oppo_marker.color.b = 0.09

        self.oppo_xacro_pub.publish(oppo_marker)
    ...
    def timer_callback(self):
        ...
        ego_marker.lifetime = Duration(seconds=1, nanoseconds=20000000).to_msg()
        ...
        self.ego_xacro_pub.publish(ego_marker)
